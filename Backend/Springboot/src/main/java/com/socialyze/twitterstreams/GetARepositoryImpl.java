...
//	}
	...
	public Employee setAccessToken(String userId) {
		...
		if(token == null) {
			...
			employee.personalDetails.isOnline = true;
			mongoTemplate.save(employee, "Employee");
	...
	public boolean logout(String userId, String token) {
		...
		if(checkAccessControl(employee,token)) {
			if(employee.userDetails.getAccessToken().equals(token)) {
				...
				employee.personalDetails.isOnline = false;
				mongoTemplate.save(employee, "Employee");
				return true;
			}
		}
	...
	@Override
	public Employee updateProfile(String userId, String token, Employee employee) {
		Employee emp = getEmployee(userId);
		if(checkAccessControl(emp, token)) {
			if(employee.employeeID != null && employee.employeeID.length() > 0){
				emp.employeeID = employee.employeeID;
			}
			if(employee.personalDetails.name != null && employee.personalDetails.name.length() > 0){
				emp.personalDetails.name = employee.personalDetails.name;
			}
			if(employee.personalDetails.address != null && employee.personalDetails.address.length() > 0){
				emp.personalDetails.address = employee.personalDetails.address;
			}
			if(employee.personalDetails.email != null && employee.personalDetails.email.length() > 0){
				emp.personalDetails.email = employee.personalDetails.email;
			}
			if(employee.personalDetails.interest != null && employee.personalDetails.interest.length() > 0){
				emp.personalDetails.interest = employee.personalDetails.interest;
			}
			if(employee.personalDetails.remarks != null && employee.personalDetails.remarks.length() > 0){
				emp.personalDetails.remarks = employee.personalDetails.remarks;
			}
			if(employee.personalDetails.image != null && employee.personalDetails.image.length() > 0){
				emp.personalDetails.image = employee.personalDetails.image;
			}
			if(employee.personalDetails.designation != null && employee.personalDetails.designation.length() > 0){
				emp.personalDetails.designation = employee.personalDetails.designation;
			}
			mongoTemplate.save(emp , "Employee");
			return emp;
		}
		return null;
	}
